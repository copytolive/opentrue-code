...
async function sendAiContext(relativePath, content, instruction = '') {
  const target = guard.resolveExisting(relativePath);
  const stat = await fsp.stat(target);
  if (!stat.isFile()) throw new Error('AI context requires a local file');
  const canonicalRelative = path.relative(guard.root, target);
  const text = content == null ? await fsp.readFile(target, 'utf8') : String(content);
  const bytes = Buffer.byteLength(text, 'utf8');
  if (bytes > MAX_AI_CONTEXT_BYTES) throw new Error('file is too large for AI context (max 256 KiB)');

  const tab = tabs.get(activeTabId);
  if (!tab || tab.profileId !== activeProfileId) throw new Error('no active browser tab');
  const wc = tab.view.webContents;
  const url = wc.getURL() || tab.requestedUrl;
  ...
  const request = [
    ...
    'This content was explicitly selected by the user from the root-locked local workspace.',
    'You do NOT have access to any other local file unless the user explicitly sends it.',
    instruction ? `Instruction: ${String(instruction).trim()}` : 'Instruction: Read this file and use it as the source for the next request.',
    '',
    '--- BEGIN LOCAL FILE ---',
    text,
    '--- END LOCAL FILE ---',
  ...
  const script = `(() => {
    ...
    const selectors = provider === 'ChatGPT'
      ...
      : provider === 'Claude'
        ? ['div[contenteditable="true"].ProseMirror', 'div[contenteditable="true"][data-testid*="input"]', 'div[contenteditable="true"]', 'textarea']
    ...
    element.focus();
    const current = 'value' in element ? element.value : (element.innerText || element.textContent || '');
    const combined = current.trim() ? current + '\\n\\n' + payload : payload;
    if ('value' in element) {
      const proto = element instanceof HTMLTextAreaElement ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype;
      const setter = Object.getOwnPropertyDescriptor(proto, 'value')?.set;
      if (setter) setter.call(element, combined); else element.value = combined;
      ...
      try {
        element.dispatchEvent(new InputEvent('input', { bubbles: true, inputType: 'insertText', data: combined }));
      } catch {
        element.dispatchEvent(new Event('input', { bubbles: true }));
      }
    }
  ...
  if (!result?.ok) throw new Error('AI composer not found on the active provider page');
  return { provider, path: canonicalRelative, bytes, inserted: true, submitted: false };
}
...
function ensurePreviewView() {
  ...
  previewView.webContents.on('did-start-loading', () => {
    ...
    send('preview:state', { url, state: previewLoaded ? 'LOADING' : 'IDLE', loading: previewLoaded });
  });
  previewView.webContents.on('did-stop-loading', () => {
    const url = previewView.webContents.getURL();
    const live = previewLoaded && /^https?:/i.test(url);
    send('preview:state', { url, state: live ? 'LIVE' : 'IDLE', loading: false });
  });
  previewView.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    if (!isMainFrame || errorCode === -3) return;
    previewLoaded = false;
    send('preview:state', { url: validatedURL, state: 'ERROR', loading: false, error: errorDescription });
  });
...
function registerIpc() {
  ...
  ipcMain.handle('ai:sendContext', async (_event, relativePath, content, instruction) => sendAiContext(relativePath, content, instruction));
  ...
  ipcMain.handle('preview:load', async (_event, value) => {
    ...
    if (url === HOME_URL || !/^https?:/i.test(url)) throw new Error('preview requires an http(s) URL');
    const view = ensurePreviewView();
    previewLoaded = true;
    view.setBounds(previewBounds);
    view.setVisible(true);
    send('preview:state', { url, state: 'LOADING', loading: true });
    try {
      await view.webContents.loadURL(url);
      return { url };
    } catch (error) {
      previewLoaded = false;
      send('preview:state', { url, state: 'ERROR', loading: false, error: error.message });
      throw error;
    }
  });
  ...
  ipcMain.handle('preview:openExternal', async () => {
    if (!previewLoaded) throw new Error('preview is not loaded');
