...
test('AI bridge waits for provider composer hydration after routing', () => {
  assert.match(aiBridge, /Date\.now\(\) \+ 12000/);
  assert.match(aiBridge, /await new Promise\(\(resolve\) => setTimeout\(resolve, 200\)\)/);
  assert.match(aiBridge, /composer not found after waiting for provider UI/);
});
