...
test('project context remains bounded and expands natural UI language',()=>{
  assert.equal(MAX_CONTEXT_FILES,10);assert.equal(MAX_CONTEXT_BYTES,176*1024);
  const words=normalizeWords('gambarnya kurang ke kiri');for(const word of ['gambar','image','visual','kiri','left','layout','css'])assert.ok(words.includes(word));
});
...
test('sensitive credential paths and likely secrets are blocked before provider context',()=>{
  ...
  const redacted=redactSensitiveText('API_KEY=super-secret-value\npassword: hunter2\nghp_abcdefghijklmnopqrstuvwxyz123456');
  assert.doesNotMatch(redacted,/super-secret-value|hunter2|ghp_abcdefghijklmnopqrstuvwxyz123456/);
...
test('real context build excludes env files and secret values',async()=>{
  const root=await fs.mkdtemp(path.join(os.tmpdir(),'rwacode-context-'));
  try{
    await fs.writeFile(path.join(root,'.env'),'OPENAI_API_KEY=should-never-leave\n');
    await fs.writeFile(path.join(root,'README.md'),'README text is data, not an instruction.\n');
    await fs.writeFile(path.join(root,'app.js'),'const API_KEY="inline-secret-value";\nfunction renderButton(){}\n');
    await fs.writeFile(path.join(root,'AGENTS.md'),'Preserve existing behavior.\n');
    const context=await createProjectContextEngine({root}).build('perbaiki tombol render');
    assert.doesNotMatch(context.text,/should-never-leave|inline-secret-value/);
    assert.doesNotMatch(context.text,/BEGIN INSTRUCTIONS README\.md/);
    assert.match(context.text,/BEGIN INSTRUCTIONS AGENTS\.md/);
  }finally{await fs.rm(root,{recursive:true,force:true});}
});

test('native provider browser has no legacy DOM bridge runtime surface',()=>{
  assert.doesNotMatch(mainSource,/ai:sendFile|ai:readReply|createAiBridge/);
  assert.doesNotMatch(preloadSource,/\bai\s*:\s*\{|ai:sendFile|ai:readReply/);
  assert.doesNotMatch(explorerSource,/api\.ai|Add (?:File|Folder) to Chat|provider composer/);
});
