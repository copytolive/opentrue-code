...
(() => {
  ...
  restoreRailWidths();
  window.addEventListener('resize',()=>requestAnimationFrame(()=>{restoreRailWidths();enforcePreviewNativeBounds();}));
  api.preview.onState(()=>requestAnimationFrame(enforcePreviewNativeBounds));
  for(const button of document.querySelectorAll('.device-button[data-device]'))button.addEventListener('click',()=>requestAnimationFrame(()=>window.dispatchEvent(new Event('resize'))));

  const reload=document.getElementById('previewReloadButton');
  const go=document.getElementById('previewGoButton');
  const placeholder=document.getElementById('previewPlaceholder');
  if(reload&&go)reload.onclick=()=>{if(!placeholder?.classList.contains('hidden'))go.click();else api.preview.reload().catch(()=>{});};
  ...
  if(deviceBar&&!fullscreenButton){
    fullscreenButton=document.createElement('button');fullscreenButton.id='previewFullscreenButton';fullscreenButton.className='rw-preview-fullscreen-button';fullscreenButton.type='button';fullscreenButton.textContent='⛶ Full Screen';fullscreenButton.title='Full Screen Preview';
    const reloadButton=document.getElementById('previewReloadButton');deviceBar.insertBefore(fullscreenButton,reloadButton||null);
  }

  async function setFullscreen(next){
    fullscreen=Boolean(next);document.body.classList.toggle('preview-fullscreen',fullscreen);
    if(fullscreenButton){fullscreenButton.textContent=fullscreen?'Esc Exit Full Screen':'⛶ Full Screen';fullscreenButton.setAttribute('aria-pressed',String(fullscreen));}
    if(fullscreen){previewTab?.click();await api.browser.setVisible(false).catch(()=>{});}else{
      const address=document.getElementById('addressInput')?.value||'';
      const editorHidden=document.getElementById('editorPanel')?.classList.contains('hidden')!==false;
      if(address!=='rwacode://newtab'&&editorHidden)await api.browser.setVisible(true).catch(()=>{});
    }
  ...
  document.addEventListener('keydown',(event)=>{if(event.key==='Escape'&&fullscreen){event.preventDefault();setFullscreen(false);}});
  if(previewSurface&&typeof ResizeObserver==='function'){const observer=new ResizeObserver(()=>window.dispatchEvent(new Event('resize')));observer.observe(previewSurface);}
})();
